...
% alfa from 0.1 to 0.99 at step of 0.1
 for alfa = 0.01 : 0.01 : 0.99
     [x,fval,exitflag,output,lambda] = linprog(alfa*C(1,:)+(1-alfa)*C(2,:),A,b) ;
     plot(x(1),x(2),'g*');
     MINIMA=[MINIMA;alfa, x']; % vector of minimums with alfa associated and solution
     LAMBDA=[LAMBDA;alfa,lambda.ineqlin']; % vector of inqeualities multipliers with alfa associated and the associated constraint 
     hold on
     grid on
 end
% 
