...
% this is an initialization with random values 
% the rand itself generates number from 0 to 1
% the *10 generates number from 0 to 10

%this gives a matrix of order kX2
% so gives us 3 random vectors
...
while true
    ...
    % update centroids
    % cambia solo la formula qua
    for j = 1 : k
        ind = find(cluster == j);
        if isempty(ind)==0
            % is the median not the averge as before
    ...
    for i = 1 : l
        ...
        for j = 1 : k
            if norm(data(i,:)-x(j,:),1) < d
                % you also change the norm with the norm 1 instead
                % of the squared norm
                d = norm(data(i,:)-x(j,:),1);
                cluster(i) = j;
            end
