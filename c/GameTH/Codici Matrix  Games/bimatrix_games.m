...
function pureNashEquilibria(matrice1, matrice2)
    ...
    for j = 1:num_colonne_1  % Change loop variable to iterate over columns
        colonna = matrice1(:, j);  % Change to select the j-th column
        minimo = min(colonna);
    ...
    for i = 1:num_righe_2  % Change loop variable to iterate over rows
        riga = matrice2(i, :);  % Change to select the i-th row
        minimo = min(riga);
        indici_minimi = find(riga == minimo);
        
        for j = 1:length(indici_minimi)
            result_matrix_2 = [result_matrix_2; i, indici_minimi(j)];
        end
    end
