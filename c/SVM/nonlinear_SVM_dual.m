...
% Gaussian kernel
% we evaluate gaussian kernel for each point
gamma = 1 ;
K = zeros(l,l);
for i = 1 : l
    for j = 1 : l
...
% identical to the soft margin one

[la,ov] = quadprog(Q,-ones(l,1),[],[],y',0,zeros(l,1),C*ones(l,1));

% compute b
% search lambda in tange 0 C
% then evaluate the vector b as we determied 
...
% s is the evaluation of the function f(x) in various points
